...
{
    if (nrhs < 12) {
        mexErrMsgIdAndTxt("Altera:CIC:InputArgs", "Missing arguments.\nUsage: cic(filter_type{'i','d'}, N, M, R_max, R_min, is_size, cs_size, B_in, B_out, rounding_type{'TRUNCATE','CONV_ROUND','ROUND_UP','SATURATE','H_PRUNE'}, operation{'LOAD'}, input_matrix)");
    }
    ...
    if (operation == "LOAD") {       
        CIC_data CIC_0;
        mexPrintf("Loading data from input cell array.\n");
        CIC_0.load_data(filter_type, B_in, B_out, R_Max, R_Min, cs_size, is_size, prhs[11]);
        mexPrintf("Done \n");
        mexPrintf("Executing CIC: \n");
        CIC_0.execute_CIC(filter_type, is_size, N, M, ROUNDING_TYPE, B_in, B_out, h_prune);
        mexPrintf("Done \n");
        mexPrintf("Storing data to output array.\n");
        CIC_0.store_data(plhs, 0, B_out);
        mexPrintf("Done \n");
    }
    else {
